...
* webui 新增"日程"tab，可在网页端直接查看与管理工作日日程，不再仅依靠指令。
* 支持切换人格与日期浏览今日及历史日程，查看穿搭、时间线、天气等完整信息。
* 支持手动编辑日程内容（穿搭、摘要、时间线各时段），编辑后保存即生效。
* 支持一键重生成当日日程、按需求定制生成（输入额外要求触发定制生成）。
* 支持设置或取消明日定制要求，与 `/明日日程`、`/取消明日日程` 指令共用同一存储。
* 生成过程异步执行，进度实时可见，导航栏徽章提示生成中状态，完成后自动刷新。

**2. 🔗 修正 grok 搜索插件链接**

* README 中 grok 搜索插件链接由带后缀的 `astrbot_plugin_grok_web_search_Inoryu7z` 更正为 `astrbot_plugin_grok_web_search`，并补充说明该仓库为 fork 自上游并做了耦合魔改的版本，本地目录名可能带后缀但仓库地址以此为准。
...
* 默认风格池更新。已有的人格配置不受影响，仅空池或新增人格回退时使用新默认。
...
* 日程细分 LLM 现需将 title/detail 里指代主角的"她"转译为"我"，非主角的"她"按语义保留，角色自称（本小姐/小星/姐姐等）不转译。细分结果注入对话 LLM 系统提示词时视角一致，避免对话 LLM 看到第三人称描述。
