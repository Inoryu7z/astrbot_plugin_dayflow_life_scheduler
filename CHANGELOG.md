...
**🐛 风格研究/审查缓存隔离 + 定制日程缓存绕过 + 重载重复生成修复**

**1. 🐛 修复风格研究缓存跨人格串用**

* 风格研究与审查缓存此前仅按 `style_name` 索引，A 人格随机到某风格写入缓存后，给 B 人格生成同风格时直接命中 A 的缓存（包括审查 `reviewed` 标记），违反人格隔离。
* 新增 `_style_cache_key(persona, style)` 复合键（`persona::style`），研究写入与审查读取同人格同风格仍同键，保证经典库注入链路一致；不同人格缓存完全隔离。

**2. 🐛 修复定制日程错误走缓存**

* `/定制日程` 触发的研究命中条件此前未考虑 `extra_requirement`，定制要求直接复用旧研究结果。
* `/定制日程` 触发的审查也因 `reviewed` 标记被跳过。
* 现在定制日程强制重新研究与重新审查，不再命中缓存、不再跳过审查。

**3. 🐛 修复生成途中重载插件导致重复生成**

* `generating_personas` 生成锁原为内存态，重载即丢失；调度器（30s 一轮）发现"未消费+未生成+锁已丢"便重新触发，导致同日多生成一次。
* 生成锁现持久化到状态文件，时间基准由 `time.monotonic()` 改为墙钟 `time.time()`，重载后能识别"正在生成"状态而跳过重复触发，stale 阈值（600s）到期后自动清理并重试一次。
