...
* 新增子款式反向索引（`SUB_VARIANT_NAME_TO_STYLE`）：通过子款式名反查大风格名
* 定制日程支持指定子款式：用户发送"穿小白云"时自动索引到对应大风格
* 定制意图解析新增 `outfit_item_period`（morning/afternoon/all_day/both）和 `outfit_item_afternoon` 字段
* 子款式选择支持指定模式：强制选入用户指定的子款式，另一个随机
* 子款式注入支持全天模式：用户要求全天穿同一款时不换装
* 新增 `/子款式` 命令：查看各风格下的子款式名列表
* 风格池移除"暗黑洛丽塔"
