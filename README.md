...
- **📅 日程**：查看与管理工作日日程。支持切换人格与日期浏览今日及历史日程，查看穿搭、时间线、天气等完整信息；可手动编辑日程内容（穿搭、摘要、时间线各时段）、一键重生成当日日程、按需求定制生成、设置或取消明日定制要求。生成过程异步执行，进度实时可见。日程里的今日穿搭与午后换装都带「加入优秀库」按钮，可把某日实际生成出来的某一套直接收入优秀库（夜间居家装暂不支持收录，避免它日后被当作外出款复用）。
- **📐 设计**：手动发起一次穿搭设计。可指定人格名、风格（如"洛丽塔"/"废土风"/"杏花微雨"）、主题描述、晨间/午后单品（留空则模型自由发挥）、调用提供商。设计完成后以"待审核"状态写入优秀库。
- **⭐ 优秀库**：浏览已入册的穿搭条目，条目来源包括外部导入、webui 手动设计与日程收录。支持分级管理（`starred` 标星收藏款 / `normal` 经典款，starred 在自动生成时拥有更高被抽中权重）；每条都包含晨间与午后两套完整搭配，符合双套换装结构；可点击"立即使用此条目生成今日日程"跳过风格池抽取与 Grok 研究直接使用；支持编辑分级、备注、单品描述与删除。从日程收录时可选择直接收录原文，或先交给二次审核师改写为规范款式描述后再确认入库。
