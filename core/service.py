...
- morning_look: object，晨间第一套穿搭方案，包含：
  - pieces: string[]，单品。每条为一段完整描述，须涵盖单品名、材质、色名、版型与设计细节，具体到可直接想象出实物。**详尽度分级**：连衣裙、上装、下装等核心服装单品描述不少于100字，须包含至少2个具体设计细节（如刺绣图案、装饰件、边缘处理、层次结构等）；内搭、袜子、腰饰等辅助单品不做字数要求，但材质与设计细节仍须具体。外搭/外套等叠穿单品仅当风格本身天然包含叠穿层次时才可存在，且仅当作为核心单品时才需满足100字要求。袜子为功能型腿部穿着（丝袜、裤袜等），腿饰为装饰型（腿套、腿环等），两者不宜同时出现。禁止实用性描述（如"透气舒适""方便活动""适合XX天气"）
  - accessories: string[]，配饰。鞋为必选项（完整造型的必要组成部分），其余从包/首饰/发饰/腰带等中自由选取。每个配饰为独立单品，禁止将多件合并为一条。每个须包含品名、材质和色名，可标注设计细节
...
## 设计自查
完成两套穿搭设计后，从以下五个维度逐个审视每套方案，发现以下问题则调整后再输出：
...
class DayflowService:
    ...
    async def _review_style_payload(self, style_name: str, payload: dict[str, Any], sources: list[dict[str, str]], persona_name: str | None = None) -> tuple[str, dict[str, Any], list[dict[str, str]], bool, list[str]]:
        """二次审查风格研究产出的穿搭方案。
        ...
        logger.info(f"[dayflow-风格审查] 审查未通过，使用改进方案 | style={style_name} | issues={issues}")
